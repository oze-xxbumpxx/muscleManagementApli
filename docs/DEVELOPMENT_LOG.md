...
- **追加**: `docs/FRONTEND_ARCHITECTURE.md`（Presentational/Container、フラット `frontend/src/` 構成、ルーティング、実装フェーズ、Apollo・Codegen・キャッシュ方針の注意、**開始前の決定事項 Q1–Q10**）
- **更新**: `docs/ARCHITECTURE.md` のフロント章を上記への参照中心に整理。型戦略のパス例をフロント直下構成に合わせて修正。
- **更新**: `.cursorrules` のフロントディレクトリ例を `features/training` から実採用のフラット構成に変更。技術スタックに Vite / Router / Tailwind（予定）を追記。
- **更新**: `CLAUDE.md`・`README.md` から `FRONTEND_ARCHITECTURE.md` へリンク。
